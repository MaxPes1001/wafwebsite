...
if(isset($_POST['logi']))
{
    if(empty($_POST['pwneu'])||empty($_POST['pwalt']))
    {
       ...
       $name = $_SESSION["usern"];
        $db_res = mysqli_query($link,"SELECT * FROM `TeamLogin` WHERE `User` = '".$name."'");
    ...
    if($pwalt==$pwres) {
        ...
        mysqli_query($link, "UPDATE `TeamLogin` SET `PW`='" . $pwneu . "' WHERE `User` = '" . $name . "'");
...
<form action="pwandern.php" method="POST" class="login-form">
    ...
    <div class="txtb">
        ...
        <span data-placeholder="Altes Password"></span>
    </div>
    <div class="txtb">
